...
class BiEncoderDecoder(nn.Module):
    def __init__(self, model_forward, model_backward, d_model, decoder_output_size, DEVICE):
        super(BiEncoderDecoder, self).__init__()
        self.model_forward = model_forward
        self.model_backward = model_backward
        self.combiner = nn.Linear(2 * d_model, decoder_output_size)
        self.to(DEVICE)

    def forward(self, src_forward, src_backward, trg_forward, trg_backward):
        forward = self.model_forward(src_forward, trg_forward)
        backward  = self.model_backward(src_backward, trg_backward)
        return self.combiner(torch.cat([forward, backward], dim=1))
...
def make_model(DEVICE, num_layers, encoder_input_size, decoder_output_size, d_model, adj_mx, nb_head, num_of_weeks,
    ...
    src_dense_ahead = nn.Linear(encoder_input_size, d_model)
    src_dense_behind = nn.Linear(encoder_input_size, d_model)

    if ScaledSAt:  # employ spatial self attention
        position_wise_gcn_ahead = PositionWiseGCNFeedForward(spatialAttentionScaledGCN(norm_Adj_matrix, d_model, d_model), dropout=dropout)
        position_wise_gcn_behind = PositionWiseGCNFeedForward(spatialAttentionScaledGCN(norm_Adj_matrix, d_model, d_model), dropout=dropout)
    else:  # 不带attention
        position_wise_gcn_ahead = PositionWiseGCNFeedForward(spatialGCN(norm_Adj_matrix, d_model, d_model), dropout=dropout)
        position_wise_gcn_behind = PositionWiseGCNFeedForward(spatialGCN(norm_Adj_matrix, d_model, d_model), dropout=dropout)

    trg_dense_ahead = nn.Linear(decoder_output_size, d_model)  # target input projection
    trg_dense_behind = nn.Linear(decoder_output_size, d_model)  # target input projection
    ...
    if aware_temporal_context:  # employ temporal trend-aware attention
        attn_ss_ahead = MultiHeadAttentionAwareTemporalContex_q1d_k1d(nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, kernel_size, dropout=dropout)  # encoder的trend-aware attention用一维卷积
        attn_st_ahead = MultiHeadAttentionAwareTemporalContex_qc_k1d(nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, kernel_size, dropout=dropout)
        att_tt_ahead = MultiHeadAttentionAwareTemporalContex_qc_kc(nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, kernel_size, dropout=dropout)  # decoder的trend-aware attention用因果卷积
        attn_ss_behind = MultiHeadAttentionAwareTemporalContex_q1d_k1d(nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, kernel_size, dropout=dropout)  # encoder的trend-aware attention用一维卷积
        attn_st_behind = MultiHeadAttentionAwareTemporalContex_qc_k1d(nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, kernel_size, dropout=dropout)
        att_tt_behind = MultiHeadAttentionAwareTemporalContex_qc_kc(nb_head, d_model, num_of_weeks, num_of_days, num_of_hours, num_for_predict, kernel_size, dropout=dropout)  # decoder的trend-aware attention用因果卷积
    else:  # employ traditional self attention
        attn_ss_ahead = MultiHeadAttention(nb_head, d_model, dropout=dropout)
        attn_st_ahead = MultiHeadAttention(nb_head, d_model, dropout=dropout)
        att_tt_ahead = MultiHeadAttention(nb_head, d_model, dropout=dropout)
        attn_ss_behind = MultiHeadAttention(nb_head, d_model, dropout=dropout)
        attn_st_behind = MultiHeadAttention(nb_head, d_model, dropout=dropout)
        att_tt_behind = MultiHeadAttention(nb_head, d_model, dropout=dropout)

    if SE and TE:
        encode_temporal_position_ahead = TemporalPositionalEncoding(d_model, dropout, max_len, en_lookup_index)  # decoder temporal position embedding
        encode_temporal_position_behind = TemporalPositionalEncoding(d_model, dropout, max_len, en_lookup_index)  # decoder temporal position embedding
        decode_temporal_position_ahead = TemporalPositionalEncoding(d_model, dropout, num_for_predict)
        decode_temporal_position_behind = TemporalPositionalEncoding(d_model, dropout, num_for_predict)
        spatial_position_ahead = SpatialPositionalEncoding(d_model, num_of_vertices, dropout, GCN(norm_Adj_matrix, d_model, d_model), smooth_layer_num=smooth_layer_num)
        spatial_position_behind = SpatialPositionalEncoding(d_model, num_of_vertices, dropout, GCN(norm_Adj_matrix, d_model, d_model), smooth_layer_num=smooth_layer_num)
        encoder_embedding_ahead = nn.Sequential(src_dense_ahead, c(encode_temporal_position_ahead), c(spatial_position_ahead))
        encoder_embedding_behind = nn.Sequential(src_dense_behind, c(encode_temporal_position_behind), c(spatial_position_behind))
        decoder_embedding_ahead = nn.Sequential(trg_dense_ahead, c(decode_temporal_position_ahead), c(spatial_position_ahead))
        decoder_embedding_behind = nn.Sequential(trg_dense_behind, c(decode_temporal_position_behind), c(spatial_position_behind))
    elif SE and (not TE):
        spatial_position_ahead = SpatialPositionalEncoding(d_model, num_of_vertices, dropout, GCN(norm_Adj_matrix, d_model, d_model), smooth_layer_num=smooth_layer_num)
        encoder_embedding_ahead = nn.Sequential(src_dense_ahead, c(spatial_position_ahead))
        decoder_embedding_ahead = nn.Sequential(trg_dense_ahead, c(spatial_position_ahead))
        spatial_position_behind = SpatialPositionalEncoding(d_model, num_of_vertices, dropout, GCN(norm_Adj_matrix, d_model, d_model), smooth_layer_num=smooth_layer_num)
        encoder_embedding_behind = nn.Sequential(src_dense_behind, c(spatial_position_behind))
        decoder_embedding_behind = nn.Sequential(trg_dense_behind, c(spatial_position_behind))
    elif (not SE) and (TE):
        encode_temporal_position_ahead = TemporalPositionalEncoding(d_model, dropout, max_len, en_lookup_index)  # decoder temporal position embedding
        decode_temporal_position_ahead = TemporalPositionalEncoding(d_model, dropout, num_for_predict)
        encoder_embedding_ahead = nn.Sequential(src_dense_ahead, c(encode_temporal_position_ahead))
        decoder_embedding_ahead = nn.Sequential(trg_dense_ahead, c(decode_temporal_position_ahead))
        encode_temporal_position_behind = TemporalPositionalEncoding(d_model, dropout, max_len, en_lookup_index)  # decoder temporal position embedding
        decode_temporal_position_behind = TemporalPositionalEncoding(d_model, dropout, num_for_predict)
        encoder_embedding_behind = nn.Sequential(src_dense_behind, c(encode_temporal_position_behind,))
        decoder_embedding_behind = nn.Sequential(trg_dense_behind, c(decode_temporal_position_behind,))
    else:
        encoder_embedding_ahead = nn.Sequential(src_dense_ahead)
        decoder_embedding_ahead = nn.Sequential(trg_dense_ahead)
        encoder_embedding_behind = nn.Sequential(src_dense_behind)
        decoder_embedding_behind = nn.Sequential(trg_dense_behind)

    encoderLayer_ahead = EncoderLayer(d_model, attn_ss_ahead, c(position_wise_gcn_ahead), dropout, residual_connection=residual_connection, use_LayerNorm=use_LayerNorm)
    encoderLayer_behind = EncoderLayer(d_model, attn_ss_behind, c(position_wise_gcn_behind), dropout, residual_connection=residual_connection, use_LayerNorm=use_LayerNorm)

    encoder_ahead = Encoder(encoderLayer_ahead, num_layers)
    encoder_behind = Encoder(encoderLayer_behind, num_layers)

    decoderLayer_ahead = DecoderLayer(d_model, att_tt_ahead, attn_st_ahead, c(position_wise_gcn_ahead), dropout, residual_connection=residual_connection, use_LayerNorm=use_LayerNorm)
    decoderLayer_behind = DecoderLayer(d_model, att_tt_behind, attn_st_behind, c(position_wise_gcn_behind), dropout, residual_connection=residual_connection, use_LayerNorm=use_LayerNorm)

    decoder_ahead = Decoder(decoderLayer_ahead, num_layers)
    decoder_behind = Decoder(decoderLayer_behind, num_layers)

    generator_ahead = nn.Linear(d_model, d_model)
    generator_behind = nn.Linear(d_model, d_model)

    model_ahead = EncoderDecoder(encoder_ahead,
                           decoder_ahead,
                           encoder_embedding_ahead,
                           decoder_embedding_ahead,
                           generator_ahead,
                           DEVICE)
    model_behind = EncoderDecoder(encoder_behind,
                           decoder_behind,
                           encoder_embedding_behind,
                           decoder_embedding_behind,
                           generator_behind,
                           DEVICE)
    model = BiEncoderDecoder(model_ahead, model_behind, d_model, decoder_output_size, DEVICE)
    # param init
