...
def get_adjacency_matrix_2direction_admie(edges, edges_weights, num_of_vertices, id_filename=None):
    '''
    Parameters
    ----------
    distance_df_filename: pd.Dataframe with
    ...
    for id_row in range(edges.shape[1]):
        row = edges[:, id_row]
        i, j, distance = int(row[0]), int(row[1]), float(edges_weights[id_row])
        A[i, j] = 1
        A[j, i] = 1
        distaneA[i, j] = distance
...
def compute_val_loss(net, val_loader, criterion, sw, epoch, device=None):
    '''
...
def predict_and_save_results(net, data_loader, epoch, type, device):
    '''
    for transformerGCN
    :param net: nn.Module
    ...
    with torch.no_grad():
        ...
        for batch_data in tqdm(data_loader):
            ...
            data_target_tensor.append(labels.numpy())
            if batch_index % 100 == 0:
        ...
        data_target_tensor = np.concatenate(data_target_tensor, 0)  # (batch, N, T', 1)
