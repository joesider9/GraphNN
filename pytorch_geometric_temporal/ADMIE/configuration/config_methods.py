...
from ADMIE.configuration.config_utils import define_n_jobs
from ADMIE.configuration.config_utils import find_pycharm_path
from ADMIE.configuration.config_utils import define_enviroment
from ADMIE.configuration.config_project import TYPE
from ADMIE.configuration.config_project import HORIZON_TYPE
from ADMIE.configuration.config_input_data import NWP_DATA_MERGE
from ADMIE.configuration.config_input_data import DATA_COMPRESS
from ADMIE.configuration.config_input_data import DATA_ROW_SCALE
from ADMIE.configuration.config_input_data import DATA_NWP_SCALE
from ADMIE.configuration.config_input_data import DATA_IMG_SCALE
from ADMIE.configuration.config_input_data import DATA_IMG
from ADMIE.configuration.experiments import experiments

RUNTIME_BACKEND = 'TORCH'  # TF_2, TORCH

from ADMIE.configuration.timm_model_list import timm_models
from ADMIE.configuration.huggingface_vit_list import huggingface_models

vit_models = huggingface_models.union(timm_models)
...
DATA_TYPE_TO_CLUSTER_COMBINE = {'scale_row_method': DATA_ROW_SCALE[0],
                                ...
                                'scale_img_method': DATA_IMG_SCALE[0],
                                'merge': NWP_DATA_MERGE[0],
                                'compress': DATA_COMPRESS[0],
                                'image_data_type': DATA_IMG[0]
                                }


def rbf_variables():
    if TYPE == 'pv':
        var_imp = [['hour', 'q50_rgb_snow_250_-0.5', 'q50_ir039_250_-0.5']]
    elif TYPE == 'wind':
        var_imp = [['SWIN_ahead', 'SWIN_behind']]
    elif TYPE == 'load':
...
def config_methods():
    ...
    static_data['keep_best_models'] = 3
    static_data['CVs'] = 2
    ...
    static_data['CNN'] = {'experiment_tag': {'cnn3'},
                          'n_trials': 5,
                          'filters': 12,
                          'stride': 1,
                          'conv_dim': 3,
    ...
    static_data['CrossViVit'] = {'experiment_tag': {'CrossViVit_net'},
                                 ...
                                 'scale_nwp_method': DATA_NWP_SCALE[0],
                                 'scale_img_method': set(DATA_IMG_SCALE),
                                 'image_data_type': set(DATA_IMG),
                                 'feature_selection_method': None,
                                 'use_data': 3,
    ...
    static_data['TIMM'] = {'experiment_tag': {'timm_net1a'},
                           'n_trials': 32 * len(vit_models),
                           'model_name': vit_models,
                           'timm_models': ' '.join(timm_models),
                           'pretrained': {'tune'},  # 'initial', 'tune', 'finetune' better 'tune
                           'sat_image_type': ['rgb_snow', 'ir039_ir108_vis006', 'RBG'],
                           'clip_value':{True, False},
                           'final_image_size': 160,
                           'transfer_learning_from_global': True,
                           'optimizer': 'adam',
                           'scheduler': 'ReduceLROnPlateau',
                           'fix_grid_search': False,
                           'merge': None,
                           'compress': None,
                           'scale_row_method': None,
                           'scale_nwp_method': None,
                           ...
                           'image_data_type': set(DATA_IMG),
                           'create_image_dataset_real_time': True,
                           'spatal_coord': {True, False},
                           'area_adjust': {'eumetdac': 50, 'eumetview': 80},
    ...
    static_data['MLP'] = {'experiment_tag': {'mlp3', 'mlp4', 'mlp5', 'mlp6'}, #'mlp1', 'mlp2', 'mlp3', 'mlp4',
                          'n_trials': 12,
                          'hold_prob': 1,
                          'batch_size': 1024,
                          'fix_grid_search': False,
    ...
    static_data['CatBoost'] = {'n_trials': 15,
                               'iterations': 1000,
    ...
    static_data['lightgbm'] = {'n_trials': 20,
                               'num_iterations': 1000,
                               'boosting_type':'gbdt',
                               'learning_rate': {0.01, 0.05, 0.1},
                               'reg_lambda': 0,
