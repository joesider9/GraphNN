...
class DCRNN(torch.nn.Module):
    ...
    def _calculate_candidate_state(self, X, edge_index, edge_weight, H, R):
        ...
        return H_tilde
    def _temp_calculate_update_gate(self, X, edge_index, edge_weight, H):
        Z = torch.cat([X, H], dim=1)
        Z = self.temp_conv_x_z(Z, edge_index, edge_weight)
        Z = torch.sigmoid(Z)
        return Z

    def _temp_calculate_reset_gate(self, X, edge_index, edge_weight, H):
        R = torch.cat([X, H], dim=1)
        R = self.temp_conv_x_r(R, edge_index, edge_weight)
        R = torch.sigmoid(R)
        return R

    def _temp_calculate_candidate_state(self, X, edge_index, edge_weight, H, R):
        H_tilde = torch.cat([X, H * R], dim=1)
        H_tilde = self.temp_conv_x_h(H_tilde, edge_index, edge_weight)
        H_tilde = torch.tanh(H_tilde)
        return H_tilde
    ...
    def forward(
        ...
        X: torch.FloatTensor,
        edge_index: torch.LongTensor,
        temp_edge_index: torch.LongTensor,
        edge_weight: torch.FloatTensor = None,
        temp_edge_weight: torch.FloatTensor = None,
        H: torch.FloatTensor = None,
        temp_H: torch.FloatTensor = None,
    ) -> torch.FloatTensor:
        r"""Making a forward pass. If edge weights are not present the forward pass
        ...
        temp_H = self._set_hidden_state(X, temp_H)
        temp_Z = self._temp_calculate_update_gate(X, temp_edge_index, temp_edge_weight, temp_H)
        temp_R = self._temp_calculate_reset_gate(X, temp_edge_index, temp_edge_weight, temp_H)
        temp_H_tilde = self._temp_calculate_candidate_state(X, temp_edge_index, temp_edge_weight, temp_H, temp_R)
        temp_H = self._calculate_hidden_state(temp_Z, temp_H, temp_H_tilde)
        return torch.cat([H, temp_H], dim=-1)
...
class BatchedDConv(MessagePassing):
    ...
    def forward(
        ...
        if not cached_idx:
            ...
            deg_out_inv = torch.reciprocal(deg_out + 1e-12)
            deg_in_inv = torch.reciprocal(deg_in + 1e-12)
            row, col = edge_index
...
class BatchedDCRNN(torch.nn.Module):
    ...
    def _create_update_gate_parameters_and_layers(self):
        ...
        self.temp_conv_x_z = BatchedDConv(
            in_channels=self.in_channels + self.out_channels,
            out_channels=self.out_channels,
            K=self.K,
            bias=self.bias,
    ...
    def _create_reset_gate_parameters_and_layers(self):
        ...
        self.temp_conv_x_r = BatchedDConv(
            in_channels=self.in_channels + self.out_channels,
            out_channels=self.out_channels,
            K=self.K,
            bias=self.bias,
    ...
    def _create_candidate_state_parameters_and_layers(self):
        ...
        self.temp_conv_x_h = BatchedDConv(
            in_channels=self.in_channels + self.out_channels,
            out_channels=self.out_channels,
            K=self.K,
            bias=self.bias,
    ...
    def _temp_calculate_update_gate(self, X, temp_edge_index, temp_edge_weight, temp_H, cached):
        temp_Z = torch.cat([X, temp_H], dim=1)
        temp_Z = self.temp_conv_x_z(temp_Z, temp_edge_index, temp_edge_weight, cached_idx=cached)
        temp_Z = torch.sigmoid(temp_Z)
        return temp_Z

    def _temp_calculate_reset_gate(self, X, temp_edge_index, temp_edge_weight, temp_H, cached):
        temp_R = torch.cat([X, temp_H], dim=1)
        temp_R = self.temp_conv_x_r(temp_R, temp_edge_index, temp_edge_weight, cached_idx=cached)
        temp_R = torch.sigmoid(temp_R)
        return temp_R

    def _temp_calculate_candidate_state(self, X, temp_edge_index, temp_edge_weight, temp_H, temp_R, cached):
        temp_H_tilde = torch.cat([X, temp_H * temp_R], dim=1)
        temp_H_tilde = self.temp_conv_x_h(temp_H_tilde, temp_edge_index, temp_edge_weight, cached_idx=cached)
        temp_H_tilde = torch.tanh(temp_H_tilde)
        return temp_H_tilde
    ...
    def forward(self, X, lag_edge_index, lag_edge_weight, edge_index, edge_weight):
        """
        ...
        temp_hidden_state = torch.zeros(batch_size, num_nodes, self.out_channels).to(X.device)
        ...
        or not torch.equal(self._cached_edge_index, edge_index) or not torch.equal(self._cached_edge_weight, edge_weight):
            ...
            self._cached_lag_edge_index = lag_edge_index
            self._cached_lag_edge_weight = lag_edge_weight
            self._cached_edge_index = edge_index
            self._cached_edge_weight = edge_weight

            # cache
            self._cached_expanded_lag_edge_index = rearrange(lag_edge_index, 'b f Ng -> f (b Ng)')
            self._cached_expanded_lag_edge_weight = rearrange(lag_edge_weight, 'b Ng -> (b Ng)')
        ...
        for t in range(seq_length):
            ...
            temp_H = temp_hidden_state.reshape(batch_size * num_nodes, self.out_channels)
            temp_Z = self._temp_calculate_update_gate(x_t, self._cached_expanded_lag_edge_index, self._cached_expanded_lag_edge_weight,
                                                      temp_H, self._cached_idx)
            temp_R = self._temp_calculate_reset_gate(x_t, self._cached_expanded_lag_edge_index,
                                                     self._cached_expanded_lag_edge_weight, temp_H, self._cached_idx)
            temp_H_tilde = self._temp_calculate_candidate_state(x_t, self._cached_expanded_lag_edge_index,
                                                                self._cached_expanded_lag_edge_weight,
                                                                temp_H, temp_R, self._cached_idx)
            temp_H = self._calculate_hidden_state(temp_Z, temp_H, temp_H_tilde)

            temp_hidden_state = temp_H.reshape(batch_size, num_nodes, self.out_channels)

        outputs = torch.cat([hidden_state, temp_hidden_state], dim=-1)
