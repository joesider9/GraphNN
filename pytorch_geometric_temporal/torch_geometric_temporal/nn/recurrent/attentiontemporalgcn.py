...
class A3TGCN2(torch.nn.Module):
    ...
    def _setup_layers(self):
        ...
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self._attention = torch.nn.Parameter(torch.empty(self.periods, device=device))
        self._lag_attention = torch.nn.Parameter(torch.empty(self.periods, device=device))
        torch.nn.init.uniform_(self._attention)

    def forward( 
        self, 
        X: torch.FloatTensor,
        lag_edge_index: torch.LongTensor,
        lag_edge_weight: torch.FloatTensor,
        edge_index: torch.LongTensor,
        edge_weight: torch.FloatTensor = None,
        lag_H: torch.FloatTensor = None,
        H: torch.FloatTensor = None
    ) -> torch.FloatTensor:
        """
        Making a forward pass. If edge weights are not present the forward pass
        ...
        lag_H_accum = 0
        probs = torch.nn.functional.softmax(self._attention, dim=0)
        lag_probs = torch.nn.functional.softmax(self._lag_attention, dim=0)
        for period in range(self.periods):
            H_accum = H_accum + probs[period] * self._base_tgcn( X[:, :, :, period], edge_index, edge_weight, H) #([32, 207, 32]
            lag_H_accum = lag_H_accum + lag_probs[period] * self._lag_base_tgcn( X[:, :, :, period], lag_edge_index, lag_edge_weight, lag_H) #([32, 207, 32]
        return torch.cat([H_accum, lag_H_accum], dim=-1)