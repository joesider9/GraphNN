...
class ADMIEDataset(Dataset):
    def __init__(
        ...
        self.park_names = data['park_names'][group]
        self.dates = data['dates']
        self.X = dict()
        norm_x = np.array([30, 360])[np.newaxis, np.newaxis, :]
        self.X = torch.from_numpy(np.clip(data['X'][:, group, :] / norm_x, 0, 1)).float()
        norm_y = np.array(rated[group])[np.newaxis, :, np.newaxis]
        self.y = torch.from_numpy(np.clip(data['y'][:, group, :] / norm_y, 0, 1)).float()
        self.spatial = data['spatial'].iloc[group]
        self.indices = [i for i in range(n_lags + 1, self.X.shape[0] - n_pred_lags-1) if not torch.isnan(self.X[i-n_lags:i+n_pred_lags]).any()
