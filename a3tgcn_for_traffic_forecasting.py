...
from pytorch_geometric_temporal.torch_geometric_temporal.nn.recurrent import BatchedDCRNN as DCRNN

from pytorch_geometric_temporal.torch_geometric_temporal.nn.recurrent import A3TGCN2, GConvLSTM, DyGrEncoder, MPNNLSTM
from torch.utils.data import DataLoader
from ADMIE.configuration.config_input_data import (variables, TARGET_VARIABLE)
from pytorch_geometric_temporal.torch_geometric_temporal.dataset.admie_ds import ADMIEDataset
...
parks = [parks[i] for i in park_group[park_group_name]]
...
for method in methods:
    ...
    dataloader_train = DataLoader(dataset_train, shuffle=shuffle, batch_size=batch_size, collate_fn=collate_fn, num_workers=16)
    dataloader_val = DataLoader(dataset_val, shuffle=shuffle, batch_size=batch_size, collate_fn=collate_fn, num_workers=16)
    dataloader_test = DataLoader(dataset_test, shuffle=shuffle, batch_size=batch_size, collate_fn=collate_fn, num_workers=16)
