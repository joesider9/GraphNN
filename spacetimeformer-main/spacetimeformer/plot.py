...
class PredictionPlotterCallback(pl.Callback):
    ...
    def on_validation_end(self, trainer, model):
        ...
        joblib.dump(pred_plots, f'pred_plot_{trainer.global_step}.pickle')
...
class ImageCompletionCallback(pl.Callback):
    ...
    def on_validation_end(self, trainer, model):
        ...
        joblib.dump(ImgCompletion, f'ImgCompletion_plot_{trainer.global_step}.pickle')
...
class CopyTaskCallback(pl.Callback):
    ...
    def on_validation_end(self, trainer, model):
        ...
        joblib.dump(CopyTask, f'CopyTask_plot_{trainer.global_step}.pickle')
...
class AttentionMatrixCallback(pl.Callback):
    ...
    def on_validation_end(self, trainer, model):
        ...
        if self_attns is not None:
            ...
            self_attn = {"test/self_attn": self_attn_imgs, "global_step": trainer.global_step}
            joblib.dump(self_attn, f'self_attn_plot_{trainer.global_step}.pickle')

        if cross_attns is not None:
            cross_attn_imgs = self._make_imgs(
                cross_attns, f"Cross Attn, Layer {self.layer},"
            )
            cross_attn = {"test/cross_attn": cross_attn_imgs, "global_step": trainer.global_step}
            joblib.dump(cross_attn, f'cross_attn_plot_{trainer.global_step}.pickle')
        ...
        pos_embs = {"test/pos_embs": emb_sim_imgs, "global_step": trainer.global_step}


        joblib.dump(pos_embs, f'pos_embs_plot_{trainer.global_step}.pickle')
