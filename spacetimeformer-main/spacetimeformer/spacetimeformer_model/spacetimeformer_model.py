...
class Spacetimeformer_Forecaster(stf.Forecaster):
    ...
    def classification_loss(
        ...
        acc = torchmetrics.functional.accuracy(
            torch.max(torch.softmax(logits, dim=1), 1).values,
            labels,
            "multiclass", num_classes=int(d_y)
        )
    ...
    def on_on_validation_epoch_end(self, outs):
        total = 0
        count = 0
        for dict_ in outs:
    ...
    def add_cli(self):
        ...
        p.update({"start_token_len": 0,  # "Length of decoder start token. Adds this many of
                  # the final context points to the start of the target sequence."
                  "d_model": 10,  # "Transformer embedding dimension."
                  "d_qk": 10,
                  "d_v": 10,
                  "n_heads": 1,  # "Number of self-attention heads."
                  "enc_layers": 1,  # "Transformer encoder layers."
                  "dec_layers": 1,  # "Transformer decoder layers."
                  "d_ff": 40,  # "Dimension of Transformer up-scaling MLP layer. (often 4 * d_model,"
                  "attn_factor": 2,  # "ProbSparse attention factor. N/A to other attn mechanisms."
                  "dropout_emb": 0.2, # "Embedding dropout rate. Drop out elements of the embedding vectors during
                  # training.",
                  "dropout_attn_matrix": 0.0, # "Attention dropout rate. Dropout elements of the attention matrix. Only
                  # applicable to attn mechanisms that explicitly compute the attn matrix (e.g. Full,.",
                  "dropout_qkv": 0.0, # "Query, Key and Value dropout rate. Dropout elements of these attention vectors
                  # during training."
                  "dropout_ff": 0.1,  # "Standard dropout applied to activations of FF networks in the Transformer.",
                  "dropout_attn_out": 0.0,
                  "global_self_attn": "performer",  # [ "full", "prob", "performer", "none", ],
                  # "Attention mechanism type."
                  "global_cross_attn": "performer",  # [ "full", "performer", "none", ],
                  # "Attention mechanism type."
                  "local_self_attn": "performer",  # [ "full", "prob", "performer", "none", ],
                  # "Attention mechanism type.",
                  "local_cross_attn": "performer",  # choices=[ "full", "performer", "none", ],
                  # "Attention mechanism type.",
                  "activation": "gelu",  # choices=["relu", "gelu"], "Activation function for Transformer encoder
                  # and decoder layers."
                  "norm": "batch", # choices=["layer", "batch", "scale", "power", "none"],
                  "init_lr": 1e-10,  # "Initial learning rate."
                  "base_lr": 5e-4, # "Base/peak LR. The LR is annealed to this value from init_lr over warmup_steps
                  # training steps.",
                  "warmup_steps": 0,  # "LR anneal steps."
                  "decay_factor": 0.25,  # "Factor to reduce LR on plateau (after warmup period is over,.",
                  "initial_downsample_convs": 0, # "Add downsampling Conv1Ds to the encoder embedding layer to reduce
                  # context sequence length.",
                  "class_loss_imp": 0.1, # "Coefficient for node classification loss function. Set to 0 to disable
                  # this feature. Does not significantly impact forecasting results due to detached gradient.",
                  "recon_loss_imp": 0.0,
                  "intermediate_downsample_convs": 0,  # "Add downsampling Conv1Ds between encoder layers.",
                  "time_emb_dim": 6, # "Time embedding dimension. Embed *each dimension of x* with this many learned
                  # periodic values.",
                  "performer_kernel": "relu",  # choices=["softmax", "relu"],"Performer attention kernel.
                  # See Performer paper for details.",
                  "performer_redraw_interval": 100, # "Training steps between resampling orthogonal random features
                  # for FAVOR+ attention",
                  "embed_method": "spatio-temporal", # choices=["spatio-temporal", "temporal"]"Embedding method.
                  # spatio-temporal enables long-sequence
                  # spatio-temporal transformer mode while temporal recovers default architecture.",
                  "attn_time_windows": 1,
                  "use_shifted_time_windows": False,
                  "pos_emb_type": "abs", #choices=["abs", "t2v"]
                  "no_final_norm": False,
                  "no_val": False,
                  "no_time": False,
                  "no_space": False, # "Remove `space (variable,` embeddings. Has no effect when using standard
                  # temporal
                  # attention. Is very likely to hurt performance w/ spatiotemporal attention",
                  "no_given": False, # "Remove `given` embeddings. Only relevant when using a start token, padding,
                  # reconstruction dropout, or null values",
                  "recon_mask_skip_all": 1.0, # "Skip all kinds of reconstruction (context sequence, dropout at this
                  # probability. 1.0 turns off all data dropout",
                  "recon_mask_max_seq_len": 5, # "Max sequence length that will be dropped from the context sequence
                  # during reconstruction dropout",
                  "recon_mask_drop_seq": 0.2, # "Pct of the context sequence that will be masked by subsequence
                  # dropout during reconstruction",
                  "recon_mask_drop_standard": 0.1, # "Pct of the total context sequence masked by standard droput
                  # (each element dropped ind., during reconstruction",
                  "recon_mask_drop_full": 0.05 # "Pct of timesteps in the context sequence that will be completely
                  # masked during reconstruction (vs. `recon_mask_drop_standard` which can mask some but not all vars at a given timestep,",
                  })
        return p
