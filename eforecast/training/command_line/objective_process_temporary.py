...
method = 'TIMM'
cluster_name = 'global'
path_cluster =  ('/media/sider/data/Dropbox/data_transfer/models/PPC/PPC_sat_ver1/pv/Ptolemaida/multi-output/model_ver0/global')
gpu_id = 0
refit = bool(0)
#
tl_path_weights = ''
    ...
    def fit_trial(self, trial_number, trials, gpu_i, fix_grid=False):
        ...
        if self.static_data[method].get('fix_grid_search', False):
            ...
            if os.path.exists(file_grid):
                grid = pd.read_csv(file_grid, header=0).replace({float('nan'): None}).to_dict('records')
            else:
                grid = self.grid_()
...
if __name__ == '__main__':
    ...
    # project_name_list = ['Ptolemaida', 'short_term']
