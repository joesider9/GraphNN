...
from eforecast.datasets.image_data.image_dataset_real_time import ImageDatasetRealTime
...
def feed_data_eval(data, indices=None, device=None):
    x = dict()
        ...
        for name in data.keys():
            if isinstance(data[name], dict):
                ...
                for name1 in data[name].keys():
                    ...
                    x[name][name1] = torch.from_numpy(values).float().to(device)
            else:
                if indices is None:
                ...
                x[name] = torch.from_numpy(values).float().to(device)
    return x
            ...
            for name in data.keys():
                if isinstance(data[name], dict):
                    ...
                    for name1 in data[name].keys():
                        ...
                        self.x[name][name1] = torch.from_numpy(values).to(self.device)
                else:
                    values = data[name] if isinstance(data[name], np.ndarray) else data[name].values
                    self.x[name] = torch.from_numpy(values).to(self.device)
        else:
            self.x['input'] = torch.from_numpy(data).to(self.device)
        self.y = torch.from_numpy(target).to(self.device)
...
class DatasetImageDict(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            y = self.y[indices].float().to(self.device) if self.y is not None else images["target"].float().to(
                self.device)
            return x, y
        else:
...
def feed_image_dataset_real_time(static_data, data, target, dates, params, device, batch_size=1, shuffle=False,
    ...
    dataloader = DataLoader(dataset, shuffle=shuffle, batch_size=batch_size, collate_fn=collate_fn_train
                                                                    if train and target is not None else collate_fn_eval)
    return dataloader
