...
class DataFeeder:
    ...
    def load_images(self, scale=None, inverse=False):
        dates_image, _ = self.files_manager.check_if_exists_image_data()
        if dates_image is None:
    ...
    def load_stats(self, scale=None, inverse=False):
        _, data_stats = self.files_manager.check_if_exists_image_data()
        if data_stats is None:
            raise ImportError(f'Cannot find stats dataset')
        else:
            for what_data, data in data_stats.items():
                if data is None:
                    continue
                dataset_name = f'data_img_{scale}_{what_data}'
                if data_stats is not None:
                    data_stats[what_data], _ = self.pipeline.transform_pipe(data, dataset_name, inverse=inverse)
        return data_stats
    ...
    def load_dataset(self, data_tag, merge=None, compress=None, scale_row_method=None, scale_nwp_method=None,
                     scale_img_method=None, img_data_type=None, inverse=False, get_all=False, transform_calendar=False):
        calendar_vars = {'month': 12, 'day': 30, 'dayweek': 7, 'hour': 24, 'minute': 60}
        if data_tag == 'row_data':
            ...
            if transform_calendar and data['calendar'] is not None:
                dfs = []
            ...
            tag = 'calendar' if data['calendar'] is not None else 'row_obs'
            metadata = {'dates': data[tag].index}
        elif data_tag == 'nwp':
            ...
            data = self.load_stats(scale=scale_img_method, inverse=inverse)
            data = self.pipeline.merge_img_dataset(data, img_types=img_data_type)
            if not isinstance(data, dict):
                metadata = {'dates': data.index}
            else:
    ...
    def get_row_data(self, what_data, merge=None, compress=None, scale_row_method=None, scale_nwp_method=None,
                     scale_img_method=None, image_data_type = None, inverse=False, get_all=False, transform_calendar=False):
        ...
        for tag in data_tags:
            if tag in {'calendar', 'obs'}:
                ...
                if data is None:
                    continue
                if tag == 'all':
                    datasets.append(pd.concat([data[tag] for tag in ['row_obs', 'calendar']
                                               if tag in data.keys()], axis=1))
                else:
                    datasets.append(data[f'row_{tag}'] if tag in {'obs'} else data['calendar'])
                ...
                data, metadata = self.load_dataset('stats', img_data_type=image_data_type,
                                                   scale_img_method=scale_img_method, inverse=inverse)
                datasets.append(data)
                metadatasets.update(metadata)
            elif tag == 'all':
                data, metadata = self.load_dataset('row_data', scale_row_method=scale_row_method,
                                                   inverse=inverse, transform_calendar=transform_calendar)
                datasets.append(pd.concat([data[tag]for tag in ['row_obs', 'row_stats', 'calendar']
                                               if tag in data.keys()], axis=1))
                metadatasets.update(metadata)
    ...
    def feed_inputs(self, data_tag, merge=None, compress=None, scale_nwp_method=None, scale_row_method=None,
                    scale_img_method=None, image_data_type=None,
        ...
        feature_selection_method = None if isinstance(feature_selection_method, float) else feature_selection_method
        if data_tag not in self.dataset_tags and compress != 'minimal':
            raise ValueError(f'Unknown dataset tag: {data_tag}')
        if 'row' in data_tag:

            data, metadata = self.get_row_data(data_tag, image_data_type=image_data_type,
            ...
            if feature_selection_method is not None:
                ...
                data = self.feature_selector.transform(feature_selector_tag, cluster["cluster_path"], data)
            ##TODO: This is a hack Should remove in the future
            if self.static_data['project_owner'] == 'ADMIE':
                data_new = dict()
                data_new['past'] = dict()
                data_new['future'] = dict()
                dict_df = data['past']
                for k, df in dict_df.items():
                    past_cols = []
                    future_cols = []
                    for col in df.columns:
                        if 'mean-' in col or 'behind' in col:
                            past_cols.append(col)
                        else:
                            future_cols.append(col)
                    data_new['past'][k] = df[past_cols]
                    future_cols = future_cols[::-1]
                    data_new['future'][k] = df[future_cols]
                data = data_new
            ##TODO: The above is a hack Should remove in the future
            past_obs = []
            past_data = []
            future_data = []
            past_calendar = []
            ...
            for key, value in data.items():
                for key1 in sorted(value.keys()):
                    value1 = value[key1]
                    columns_sorted = value1.columns[np.argsort([int(col.split('_lag_')[-1]) for col in value1.columns if '_lag_' in col]) ]
                    if len(columns_sorted) == len(value1.columns):
