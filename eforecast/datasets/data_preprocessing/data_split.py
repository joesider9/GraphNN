...
class Splitter:
    ...
    def split_cluster_data(self, clustered_dates, cv_mask):
        ...
        for name, value in group_data.items():
            if value['sel_dates_all'].shape[0] == 0:
                print('Better to run again clustering')
                value['sel_dates_all'] = pd.DatetimeIndex(np.random.choice(clustered_dates[name],
                                                                           size=int(2 * self.val_test_ratio * clustered_dates[name].shape[0])))
    ...
    def split(self, refit=False):
        ...
        if cv_mask is None:
            ...
            tag = 'calendar' if data_row['calendar'] is not None else 'row_obs'
            data_row = data_row[tag].dropna(axis='index')
            if data_row is None:
            ...
            if mask_train.shape[0] == 0 or mask_val.shape[0] == 0 or mask_test.shape[0] == 0:
                raise ImportError('Cannot find enough data to split. Check if data are exists Or choose another dataset')
