...
class DataPipeline:
    def __init__(self, static_data, recreate=False, online=False, train=False):
        ...
        self.scale_img_method = self.static_data['scale_img_method']
        ...
        self.img_data = self.static_data['img_data']
    ...
    def merge_img_dataset(self, img_data, img_types=None):
        return pd.concat([img_data[img_type] for img_type in img_types.split(':')], axis=1)
            ...
            if compress_type is not None:
                ...
                if self.static_data['ts_resolution'] == '15min':
                    ...
                    nwp_data_merged, dates_new = downsample_tensor(nwp_data_merged, nwp_metadata['dates'],
                                                                       resolution=self.static_data['ts_resolution'])
                    nwp_metadata['dates'] = dates_new
                return rearrange(nwp_data_merged, 'b t w h c -> b t c w h'), nwp_metadata
        else:
            return None, None
    ...
    def fit_row_data_pipe(self):
        ...
        self.scaler.fit(target, dataset_name, scale_method=self.scale_target_method)
        ...
        if data_row is not None:
            for what_data, data in data_row.items():
                ...
                for scale_method in self.scale_row_method:
                    ...
                    self.scaler.fit(data, dataset_name, scale_method=scale_method)
                    print(f"Fit imputer for data row {what_data}")
                    self.imputer.fit(data)
                    print(f"Fit sorter for data row {what_data}")
        _, data_img = self.files_manager.check_if_exists_image_data()
        if data_img is not None:
            for what_data, data in data_img.items():
                if data is None:
                    continue
                for scale_method in self.scale_img_method:
                    print(f"Fit {scale_method} scaler for img {what_data} data")
                    dataset_name = f'data_img_{scale_method}_{what_data}'
                    self.scaler.fit(data, dataset_name, scale_method=scale_method)
                    print(f"Fit imputer for data img {what_data}")
                    self.imputer.fit(data)
                    print(f"Fit sorter for data img {what_data}")
        self.save()
                ...
                for area, area_data in data_nwp.items():
                    for variable, var_data in area_data.items():
                        for vendor, nwp_provide_data in var_data.items():
                            ...
                            self.scaler.fit(data, dataset_name, scale_method=method)
                            data = self.scaler.transform(data, dataset_name)
