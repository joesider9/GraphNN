...
class DataTransformer:
    def __init__(self, static_data, recreate=False, is_online=False, train=False):
        ...
        self.variable_source = {var_data['name']: var_data['source']
                                for var_data in self.static_data['variables']
                                if var_data['transformer'] is not None}
    ...
    def image_clear_sky(self, dates):
        loc = Location(self.coord[0], self.coord[1], 'Europe/Athens', 0, 'kjhkjhk')
        dates = pd.date_range(start=dates[0] - pd.DateOffset(hours=24),
                              end=dates[-1] + pd.DateOffset(hours=24),
                              freq='15min')
        ck1 = loc.get_clearsky(dates - pd.DateOffset(hours=0.5))['ghi'] / 200
        ck1.index = ck1.index.tz_localize('UTC')
        ck1.index = ck1.index.tz_convert("Europe/Athens")
        d1 = [pd.to_datetime(dt.strftime('%d%m%y%H%M'), format='%d%m%y%H%M') for dt in ck1.index]
        ck1.index = pd.DatetimeIndex(d1)
        ck_max = ck1.resample('D').max().reindex(ck1.index, method='pad')
        ck_min = ck_max / 2
        ck1[ck1 < ck_min] = ck_min
        ghi = ck_max.divide(ck1)
    ...
    def fit(self, data, variable, data_dates=None):
        ...
        for transformation in transformations:
            if transformation not in {'inverse', 'brightContrast', 'sum', 'eq_histogram', 'grey', 'sam'}:
                if transformation == 'clear_sky':
                    ...
                    self.transformers[variable][transformation] = {'max': ghi.max(),
                                                         'values': ghi}
                elif transformation == 'pv_clear_sky':
                    if isinstance(data, pd.Series):
                        data = data.to_frame()
                    if isinstance(data, pd.DataFrame):
                        dates = data.index
                    ...
                    self.transformers[variable][transformation] = {'max': ghi.max(),
                                                         'values': ghi}
                elif transformation == 'img_clear_sky':
                    if isinstance(data, pd.Series):
                        data = data.to_frame()
                    if isinstance(data, pd.DataFrame):
                        dates = data.index
                        ...
                        if data_dates is None:
                            raise ValueError('If data is not dataframe, data_dates should be provided')
                        dates = data_dates
                    self.transformers[variable][transformation] = self.image_clear_sky(dates)
                elif transformation == 'resize':
                    self.transformers[variable][transformation] = self.transformer_params[variable]['resize']
                elif transformation == 'area_adjust':
                    self.transformers[variable][transformation] = dict()
                    source = self.variable_source[variable]
                    self.transformers[variable][transformation]['area_adjust'] = self.transformer_params[variable]['area_adjust'][0]
                    self.transformers[variable][transformation]['lat'] = self.static_data['site_indices'][source][0]
                    self.transformers[variable][transformation]['long'] = self.static_data['site_indices'][source][1]
                    ...
                    params = self.transformer_params[variable]
                    self.transformers[variable][transformation] = params['normalize']
    ...
    def transform(self, data, variable, data_dates=None, inverse=False):
        ...
        for transformation in transformations:
            ...
            if transformation == 'clear_sky':
                ...
                rate = 1 - (0.33 * (cs_new - cs_new.min().values[0]) / (cs_new.max().values[0] - cs_new.min().values[0]))
                data_new = []
                ...
                try:
                    if data_dates is None:
                        raise ValueError('Data_dates should be provided')
                    dates = data_dates
                    ghi = self.transformers[variable][transformation].copy()
                    dates_diff = dates.difference(ghi.index)
                    if dates_diff.shape[0] > 0:
                        ghi_new = self.image_clear_sky(dates_diff)
                        ghi = pd.concat([ghi, ghi_new])
                        ghi = ghi.sort_index()
                    ghi = ghi[~ghi.index.duplicated()]
                    ghi = ghi.loc[dates]
                    ...
                    data = data * repeat(np.array([ghi.values[0]]), '1 -> c w h', c=C, w=W, h=H)
                except:
                    return None
            elif transformation == 'area_adjust':
                area_adjust = self.transformers[variable][transformation]['area_adjust']
                lat = self.transformers[variable][transformation]['lat']
                long = self.transformers[variable][transformation]['long']
                data = data[:,
                            np.maximum(lat - area_adjust, 0):np.minimum(lat + area_adjust, data.shape[1]),
                            np.maximum(long - area_adjust, 0):np.minimum(long + area_adjust, data.shape[2])]
                data = data.astype(np.float32)
            elif transformation == 'crop':
                ...
                if data.shape[2] != resize[0] or data.shape[1] != resize[1]:
                    data = np.concatenate(
                        [np.expand_dims(cv2.resize(data[i, :, :],
                                                   dsize=resize,
                                                   interpolation=cv2.INTER_AREA), axis=0)
                ...
                value_max = self.transformers[variable][transformation]
                data1 = copy.deepcopy(data)
                data_transformed = []
                for n in range(data1.shape[0]):
                    img_norm = np.clip((255 * data1[n] / value_max[n]), 0, 255).astype(np.uint8)
                    data_transformed.append(img_norm)
                data = np.array(data_transformed).astype(np.uint8)
            elif transformation == 'norm_minmax':
