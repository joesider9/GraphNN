...
class FeatureSelector:
    ...
    def transform(self, selector_tag, cluster_path, data, transform_calendar=False):
        ...
        if 'lstm' in selector_tag:
            ...
            past_shape = data['past'][list(data['past'].keys())[0]].shape[1] if len(data['past']) > 0 else 0
            future_shape = data['future'][list(data['future'].keys())[0]].shape[1] if len(data['future']) > 0 else 0
