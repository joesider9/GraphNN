...
def create_centroids(X_train, y, params):
    ...
    if X_train.shape[0] > 100000:
        X_train = X_train.resample('15min').mean().dropna(axis='index', how='any')
        y = y.resample('15min').mean().dropna(axis='index', how='any')
    ...
    ind_x = np.random.choice(np.arange(X_tr.shape[0]), max_samples_rbf) if X_tr.shape[0] > max_samples_rbf else np.arange(X_tr.shape[0])
    ind_v = np.random.choice(np.arange(X_val.shape[0]), max_samples_rbf) if X_val.shape[0] > max_samples_rbf else np.arange(X_val.shape[0])
    for w in np.linspace(0.8, 1.4, 6):
        ...
        c.fit(X_tr[ind_x], y_tr[ind_x], X_val[ind_v], y_val[ind_v])
        if c.err < inertia:
        ...
        for i, layer in enumerate(branch):
            if 'conv' in layer[0]:
                if conv_dim is not None:
                    if horizon_type == 'multi-output' and group not in {'images'}:
