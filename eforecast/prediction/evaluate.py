...
class Evaluator:
    ...
    def inverse_target(self, y):
        y = self.scaler.inverse_transform_data(y, f'target_{self.scale_target_method}')
        y = self.row_transformer.transform(y, inverse=True)
        return y
    ...
    def evaluate_methods_for_cluster(self, clusterer_method=None, cluster_name=None, trial=None):
        ...
        for method in methods_predictions.keys():
            ...
            pred_train = self.inverse_target(pred_train_scaled)
            if pred_eval_scaled.shape[0] != 0:
                pred_eval = self.inverse_target(pred_eval_scaled)
            else:
            ...
            if pred_eval_scaled.shape[0] != 0:
                ...
                y_eval = self.inverse_target(y_eval_scaled)
            ...
            for cv_name, cv_mask in zip(cv_names, cv_masks[1:]):
                ...
                y = self.inverse_target(y_scaled)
    ...
    def evaluate_cluster_func(self, predictions, predictions_eval, y_scaled, y_eval_scaled, cluster_name):
        ...
        for method in predictions.keys():
            ...
            for col, name_col in zip(columns, target_labels):
                ...
                cv_mask = pd.DatetimeIndex(cv_mask).intersection(p_scaled.index).intersection(y_scaled.index)
                res_total_scaled = \
                ...
                for cv_name, cv_mask in zip(cv_names, cv_masks):
                    cv_mask = cv_mask.intersection(p_scaled.index).intersection(y_scaled.index)
        ...
        if 'clusters' in self.predictions.keys():
            for clusterer_method, rules in self.predictions['clusters'].items():
                for cluster_name, methods_predictions in rules.items():
                    if len(self.predictions_eval['clusters'][clusterer_method][cluster_name]) == 0:
                        continue
                    ...
                    n_top_models = self.static_data.get('keep_best_models')
                    if n_top_models is not None:
                        new_results_methods = []
                        new_results_methods_scaled = []
                        for method_temp in results_methods['method'].unique():
                            if results_methods.loc[results_methods['method'] == method_temp].shape[0] > n_top_models:
                                top_results = (results_methods.loc[results_methods['method'] == method_temp].sort_values(by=['average']).
                                               head(int(self.static_data[method_temp]['n_trials'] / 2))
                                               .sort_values(by=['mae_eval']).head(n_top_models))
                                path_method = os.path.join(self.static_data['path_model'], 'cluster_organizer',
                                                           top_results.clusterer.values[0], top_results.cluster.values[0],
                                                           method_temp)
                                remove_folders = list(set(os.listdir(path_method)).difference(
                                    set(['_'.join(trial.split('_')[-2:]) for trial in top_results.index])))
                                for f in remove_folders:
                                    shutil.rmtree(os.path.join(path_method, f))
                                new_results_methods.append(results_methods.loc[top_results.index])
                                new_results_methods_scaled.append(results_methods_scaled.loc[top_results.index])
                            else:
                                new_results_methods.append(results_methods)
                                new_results_methods_scaled.append(results_methods_scaled)
                        results_methods = pd.concat(new_results_methods)
                        results_methods_scaled = pd.concat(new_results_methods_scaled)
                    else:
                        results_methods = results_methods.sort_values(by=['method', 'mae_eval'])
                        results_methods_scaled = results_methods_scaled.sort_values(by=['method', 'mae_eval'])
        ...
        if len(self.global_methods) > 0:
            ...
            if n_top_models is not None:
                ...
                for method_temp in results_methods['method'].unique():
                    if results_methods.loc[results_methods['method'] == method_temp].shape[0] > n_top_models:
                        top_results = (
                            results_methods.loc[results_methods['method'] == method_temp].sort_values(by=['average']).head(
                                int(self.static_data[method_temp]['n_trials'] / 2))
                            .sort_values(by=['mae_eval']).head(n_top_models))
                        path_method = os.path.join(self.static_data['path_model'], 'global',
                                                   method_temp)
                        remove_folders = list(set(os.listdir(path_method)).difference(
                            set(['_'.join(trial.split('_')[-2:]) for trial in top_results.index])))
                        for f in remove_folders:
    ...
    def evaluate_clusterer(self, pred_dict, y, y_scaled):
        ...
        for clusterer_name, clusterer_pred_scaled in pred_dict.items():
            ...
            clusterer_pred = self.inverse_target(clusterer_pred_scaled)
        ...
        for clusterer_name, cluster_group_pred in pred_dict.items():
            if 'averages' in cluster_group_pred.keys():
                for method, method_pred_scaled in cluster_group_pred['averages'].items():
                    ...
                    method_pred = self.inverse_target(method_pred_scaled)
            ...
            for cluster_name, cluster_pred_scaled in cluster_group_pred.items():
                for method, method_pred_scaled in cluster_pred_scaled.items():
                    if method_pred_scaled.shape[0] == 0:
                        continue
                    method_pred_scaled, y_slice = sync_datasets(method_pred_scaled, y)
                    y_scaled_slice = y_scaled.loc[y_slice.index]
                    method_pred = self.inverse_target(method_pred_scaled)
    ...
    def evaluate_distributed(self, pred_dict, y, y_scaled):
        ...
        for distributed_name, distributed_pred_scaled in pred_dict.items():
            ...
            y_scaled = y_scaled.loc[y.index]
            distributed_pred = self.inverse_target(distributed_pred_scaled)
    ...
    def evaluate_combining_models(self, pred_dict, y, y_scaled):
        ...
        for combining_model_name, combining_model_pred_scaled in pred_dict.items():
            ...
            y_scaled = y_scaled.loc[y.index]
            combining_model_pred = self.inverse_target(combining_model_pred_scaled)
