...
class Predictor:
    def __init__(self, static_data, online=False, train=False, simple_init=False):
        self.n_jobs = None
        ...
        self.regressors = []
        if self.is_Fuzzy and not simple_init:
    ...
    def predict_regressors(self, average=False, parallel=False):
        if parallel:
            preds = Parallel(n_jobs=3)(delayed(self.predict_func)(regressor) for regressor in self.regressors)
        else:
                ...
                for cluster_name, methods_predictions in rules.items():
                    ...
                    for hor in self.horizon:
                        ...
                        if n_predictors > 1:
                            pred_methods = []
                            for method in sorted(self.methods):
                                if method not in methods_predictions.keys():
                                    continue
                                pred1 = methods_predictions[method].iloc[:, hor].to_frame()
                                pred1.columns = [method]
                                pred_methods.append(pred1)
                            if len(pred_methods) > 0:
                                pred_methods = pd.concat(pred_methods, axis=1)
                            else:
                                pred_methods = pd.DataFrame()
                                continue
                            if pred_methods.shape[0] > 0:
                                pred_methods[pred_methods < 0] = 0
                                pred_methods = pred_methods.dropna(axis='index')
                    ...
                    for hor in self.horizon:
                        ...
                        for cluster_name, methods_predictions in rules.items():
                            if cluster_name in self.clusters.keys() and \
                                ...
                                if self.predictions['clusters'][clusterer_method][cluster_name][method].shape[0] == 0:
                                    continue
